...
func MakeMap(width, height int, fileName string, verbose bool) (*globeImage, error) {
	...
	for shape.Next() {
		...
		case *shp.PolyLine:
			...
			for _, pt := range pl.Points {
				x, y := hammer(pt.X, pt.Y)
				gi.image.SetColorIndex(
					int(gi.scale*(x+gi.offsetX)),
					int(gi.scale*(gi.offsetY-y)),
					1,
				)
			}
		}
	...
	for x := -180.; x <= 180.0; x += 0.02 {
		u, _ := hammer(x, 0.0)
		gi.image.SetColorIndex(int(gi.scale*(u+gi.offsetX)), int(gi.scale*gi.offsetY), 3)
	}
...
func hammer(longitude, latitude float64) (float64, float64) {
	lambda := 0.017453293 * longitude
	phi := 0.017453293 * latitude
	denominator := math.Sqrt(1. + math.Cos(phi)*math.Cos(lambda/2.0))
	x := 2.8284270 * math.Cos(phi) * math.Sin(lambda/2.0) / denominator
	y := 1.4142135 * math.Sin(phi) / denominator
	return x / 0.017453293, y / 0.017453293
}
